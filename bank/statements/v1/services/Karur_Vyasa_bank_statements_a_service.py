...
HEADER = set(['Txn', 'Value Brn Particulars Ref. No Debit Credit',
              'Balance', 'Date', 'Date Code'])
...
class KarurVyasaBankStatementsA(object):
    """Class to analyse the data obtained from Karur Vyasa Bank Type A"""
    ...
    def __get_date(self, date_input):
        ...
        for date_regex in [r'(\d{2}/\d{2}/\d{4})']:
    ...
    def __get_first_day_balance(self):
        ...
        try:
            balance_string_list = re.findall(
                r'Opening\s*Balance\s*([0-9,]+\.\d{2})', self.pdf_text)
    ...
    def data_json(self, threshhold):
        data = {
            ...
            'bank_name': 'Karur Vyasa Bank Type A',
        }
        return data
