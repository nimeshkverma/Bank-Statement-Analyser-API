...
HEADER = set(['Date', 'Particulars', 'Chq./Ref. No',
...
class IndusIndBankStatementsB(object):
    """Class to analyse the data obtained from IndusInd Bank Type B"""
    ...
    def __get_pdf_dates(self):
        ...
        try:
            from_to_string_date_list = re.findall(
                r'(\d{2}-[a-zA-Z]{3}-\d{4}(\s+)?To(\s+)?\d{2}-[a-zA-Z]{3}-\d{4})', self.pdf_text)[0]
            for from_to_string_date in from_to_string_date_list:
                for date_string in re.findall(r'(\d{2}-[a-zA-Z]{3}-\d{4})', from_to_string_date):
