...
HEADER = set(['Sl No', 'Txn Date', 'Description', 'Cheque No',
...
class BankOfIndiaBankStatementsA(object):
    """Class to analyse the data obtained from Bank of India Bank Type B"""
    ...
    def __get_pdf_dates(self):
        ...
        try:
            from_to_string_date_list = re.findall(
                r'Account\s*Statement:\s*For\s*the\s*period\s*([a-zA-Z]{3,9}\s*\d{2}\,\s*\d{4})\s*to\s*([a-zA-Z]{3,9}\s*\d{2}\,\s*\d{4})', self.pdf_text)[0]
    ...
    def data_json(self, threshhold):
        data = {
            ...
            'bank_name': 'Bank of India Type A',
        }
        return data
