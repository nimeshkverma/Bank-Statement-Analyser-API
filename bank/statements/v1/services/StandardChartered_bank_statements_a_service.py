...
ROW_AVOIDANCE_WORDS = set(['Reward Plus', 'REWARD POINTS'])

MAX_START_DAY_OF_MONTH = 5
MIN_END_DAY_OF_MONTH = 25


class StandardCharteredBankStatementsA(object):
    """Class to analyse the data obtained from Standard Chartered Bank Type A"""
    ...
    def __get_value_date(self, row):
        ...
        if len(all_date_list) >= 2:
            value_date = all_date_list[1]
        elif len(all_date_list) == 1:
            value_date = all_date_list[0]
        else:
            pass
        return value_date
    ...
    def __get_statement_set_transaction(self, index, last_date_row_index):
        statement_dict = {}
        try:
            date_row = self.raw_table_data['body'][last_date_row_index]
            row = self.raw_table_data['body'][index]
            date = self.__get_date(date_row[0])
            value_date = self.__get_value_date(date_row)
            balance = self.__get_amount(row[-1])
            if value_date is not None and balance is not None:
                statement_dict = {
                    'date': date,
                    'transaction_date': value_date,
                    'description': ' '.join(row[1:-1]),
                    'withdraw_deposit': self.__get_amount(row[-2]) if self.__get_amount(row[-2]) else 0,
                    'balance': balance,
                }
        except Exception as e:
    ...
    def __good_row(self, row):
        good_row = True
        row_string = ' '.join(row).lower()
        for avoid_word in ROW_AVOIDANCE_WORDS:
            if avoid_word.lower() in row_string:
                good_row = False
                break
        return good_row
    ...
    def __set_statements_and_transaction(self):
        ...
        index = 0
        last_date_row_index = None
        while index < statement_count:
            statement_dict = None
            row = self.raw_table_data['body'][index]
            if DATA_MIN_COLUMNS <= len(row) <= DATA_MAX_COLUMNS and self.__good_row(row):
                if self.__get_date(row[0]):
                    last_date_row_index = index
                if last_date_row_index and self.__get_amount(row[-1]) is not None:
                    statement_dict = self.__get_statement_set_transaction(
                        index, last_date_row_index)
            index += 1
    ...
    def __get_pdf_dates(self):
        ...
        from_to_string_date_list = re.findall(
            r'STATEMENT DATE\s*:\s*(\d{2} [a-zA-Z]{3} \d{4})', self.pdf_text)
    ...
    def data_json(self, threshhold):
        data = {
            ...
            'bank_name': 'Standard Chartered Type A',
        }
        return data
