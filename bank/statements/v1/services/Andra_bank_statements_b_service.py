...
HEADER = set(['Date', 'Cheque No', 'Transaction Description',
              'Withdrawals(Rs.)', 'Deposits (Rs.)', 'Balance (Rs.)'])

MAX_START_DAY_OF_MONTH = 5
MIN_END_DAY_OF_MONTH = 25


class AndraBankStatementsB(object):
    """Class to analyse the data obtained from Andra Bank Type B"""
    ...
    def __get_pdf_dates(self):
        ...
        try:
            from_to_string_date_list = re.findall(
                r'(Statement of account(\s+)?\d*(\s+)?for the period of(\s+)?\d{2}/\d{2}/\d{2}(\s+)?to(\s+)?\d{2}/\d{2}/\d{2})', self.pdf_text)[0]
            for from_to_string_date in from_to_string_date_list:
                for date_string in re.findall(r'(\d{2}/\d{2}/\d{2})', from_to_string_date):
