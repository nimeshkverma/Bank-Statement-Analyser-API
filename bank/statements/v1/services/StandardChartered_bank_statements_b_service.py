...
HEADER = set(['Date', 'Value', 'Description', 'Cheque',
...
class StandardCharteredBankStatementsB(object):
    """Class to analyse the data obtained from Standard Chartered Bank Type B"""

    def __init__(self, raw_table_data, pdf_text):
        self.raw_table_data = deepcopy(raw_table_data)
        self.pdf_text = pdf_text
        self.month_year = self.__month_year()
        self.statements = []
        self.transactions = {}
    ...
    def __default_month_year(self):
        month_year = {}
        first_month_date = datetime.date(
            datetime.date.today().year, datetime.date.today().month, 15)
        for index in xrange(1, 12):
            month_year[first_month_date.strftime("%b").lower(
            )] = first_month_date.strftime("%y").lower()
            first_month_date = first_month_date - \
                datetime.timedelta(days=30 * index)
        return month_year

    def __month_year(self):
        month_year = self.__default_month_year()
        pdf_dates = [datetime.datetime.strptime(
            pdf_date, '%d %b %Y') for pdf_date in self.__get_pdf_dates()]
        if pdf_dates:
            min_date = datetime.date(
                min(pdf_dates).year, min(pdf_dates).month, 15)
            max_date = datetime.date(
                max(pdf_dates).year, max(pdf_dates).month, 15)
            while min_date <= max_date:
                month_year[min_date.strftime("%b").lower(
                )] = min_date.strftime("%y").lower()
                min_date = min_date + datetime.timedelta(days=30)
        return month_year

    def __get_transaction_date(self, row):
        row_string = ' '.join(row)
        all_string_date_list = []
        for date_string in re.findall(r'([a-zA-Z]{3} \d{2})', row_string):
            month = date_string.split(' ')[0].lower()
            if self.month_year.get(month):
                all_string_date_list.append(
                    self.month_year[month] + ' ' + date_string)
            else:
                all_string_date_list.append('2017 ' + date_string)
        all_date_list = []
        for date_string in all_string_date_list:
            try:
                all_date_list.append(
                    datetime.datetime.strptime(date_string, '%y %b %d'))
            except Exception as e:
                pass
        if len(all_date_list) >= 2:
            return all_date_list[1]
        elif len(all_date_list) == 1:
    ...
    def __get_statement_set_transaction(self, data_list):
        statement_dict = {}
        try:
            transaction_date = self.__get_transaction_date(data_list)
            if transaction_date:
                statement_dict = {
                    'transaction_date': transaction_date,
                    'description': ' '.join(data_list[1:-1]),
                    'withdraw_deposit': self.__get_amount(data_list[-2]) if self.__get_amount(data_list[-2]) else 0,
    ...
    def __get_pdf_dates(self):
        date_text = self.pdf_text.lower().split('balance brought forward')[0]
        from_to_string_date_list = re.findall(
            r'(\d{2} [a-zA-Z]{3} \d{4})', date_text)
        return from_to_string_date_list
    ...
    def data_json(self, threshhold):
        data = {
            ...
            'bank_name': 'Standard Chartered Bank Type B',
        }
        return data
