...
HEADER = set(['Transaction', 'Value Date Description Debit Credit', 'Value Date Description Debit Credit Balance'
...
class YesBankStatementsA(object):
    """Class to analyse the data obtained from Yes Bank Type A"""
    ...
    def __get_amount_list(self, amount_input):
        all_amount_list = []
        all_string_amount_list = re.findall(r'\d+.\d{2}', amount_input)
    ...
    def __get_statement_set_transaction(self, data_list):
        ...
        try:
            statement_dict.update({
                ...
                'withdraw': 0,
                'deposit': 0,
                'balance': self.__get_amount_list(data_list[-1])[-1],
            })
            try:
                if len(data_list) == 6:
                    statement_dict['description'] = data_list[2]
                    statement_dict['withdraw'] = self.__get_amount_list(data_list[3])[
                        0]
                    statement_dict['deposit'] = self.__get_amount_list(data_list[4])[
                        0]
                else:
                    statement_dict['withdraw'] = self.__get_amount_list(' '.join(data_list))[
                        0]
                    statement_dict['deposit'] = self.__get_amount_list(' '.join(data_list))[
                        1]
            except Exception as e:
                pass
    ...
    def __get_pdf_dates(self):
        ...
        try:
            from_to_string_date_list = re.findall(
                r'(Period(\s+)?:(\s+)?[a-zA-Z]{3,9} \d{2}, \d{4}(\s+)?to(\s+)?[a-zA-Z]{3,9} \d{2}, \d{4})', self.pdf_text)[0]
            for from_to_string_date in from_to_string_date_list:
                for date_string in re.findall(r'([a-zA-Z]{3,9} \d{2}, \d{4})', from_to_string_date):
    ...
    def __get_first_day_balance(self):
        ...
        try:
            ...
            for balance_string in balance_string_list:
                balance = self.__get_amount_list(
                    re.findall(r'\d+.\d{2}', balance_string)[0])[0]
                break
        except:
            pass
