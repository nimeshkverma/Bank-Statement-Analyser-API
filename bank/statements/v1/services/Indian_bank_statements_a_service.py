...
HEADER = set(['Value', 'Post', 'Remitter',
              'Description', 'Ch', 'DR', 'CR', 'Balance'])

MAX_START_DAY_OF_MONTH = 5
MIN_END_DAY_OF_MONTH = 25


class IndianBankStatementsA(object):
    """Class to analyse the data obtained from Indian Bank Type A"""
        ...
        try:
            if partition_data == 'simple':
                statement_dict.update({
                    ...
                    'narration': ' '.join(self.raw_table_data['body'][index][1:-2]),
    ...
    def __set_statements_and_transaction(self):
        ...
        while index < statement_count:
            if not HEADER.intersection(set(self.raw_table_data['body'][index])) and self.raw_table_data['body'][index] and MIN_COLUMNS <= len(self.raw_table_data['body'][index]) <= MAX_COLUMNS:
                if re.findall(r'\d{2}/\d{2}/\d{4}', self.raw_table_data['body'][index][0]):
                    statement_dict = self.__get_statement_set_transaction(
                        index, 'simple')
                elif re.findall(r'\d{2}/\d{2}', self.raw_table_data['body'][index][0]):
                    statement_dict = self.__get_statement_set_transaction(
                        index, 'splitted')
                else:
                    pass
    ...
    def __get_pdf_dates(self):
        pdf_dates = []
        try:
            from_to_string_date_list_of_list = re.findall(
                r'(STATEMENT OF ACCOUNT from(\s+)?\d{2}/\d{2}/\d{4}(\s+)?to(\s+)?\d{2}/\d{2}/\d{4})', self.pdf_text)
            from_to_string_list = [
                from_to_string_date for from_to_string_date_list in from_to_string_date_list_of_list for from_to_string_date in from_to_string_date_list]
            for from_to_string_date in from_to_string_list:
                pdf_dates += re.findall(r'\d{2}/\d{2}/\d{4}',
                                        from_to_string_date)
        except Exception as e:
            pass
        return pdf_dates
    ...
    def __get_first_day_balance(self):
        ...
        try:
            for data_list in self.raw_table_data.get('body', []):
                if len(data_list) == 2 and data_list[0] == 'BALANCE B/F':
