...
HEADER = set(['Post Date', 'Value', 'Branch', 'Cheque',
...
class CentralBankOfIndiaBankStatementsA(object):
    """Class to analyse the data obtained from Central Bank of India Bank Type B"""
    ...
    def __get_pdf_dates(self):
        ...
        try:
            from_to_string_date_list = re.findall(
                r'STATEMENT\s*OF\s*ACCOUNT\s*from\s*(\d{2}/\d{2}/\d{4})\s*to\s*(\d{2}/\d{2}/\d{4})', self.pdf_text)[0]
    ...
    def data_json(self, threshhold):
        data = {
            ...
            'bank_name': 'Central Bank of India Type A',
        }
        return data
