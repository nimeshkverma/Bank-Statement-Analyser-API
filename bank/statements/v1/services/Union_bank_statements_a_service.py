...
HEADER = set(['Tran Id', 'Txn Date', 'Cheque No', 'Description',
...
class UnionBankStatementsA(object):
    """Class to analyse the data obtained from Union Bank Type A"""
    ...
    def __get_statement_set_transaction(self, data_list):
        ...
        try:
            statement_dict.update({
                ...
                'withdraw': self.__get_amount(data_list[-2]) if data_list[-3] in ['Dr', 'DR', 'dr'] else 0,
                'deposit': self.__get_amount(data_list[-2]) if data_list[-3] in ['Cr', 'CR', 'cr'] else 0,
    ...
    def __get_pdf_dates(self):
        ...
        try:
            from_to_string_date_list = re.findall(
                r'(Date ranging from(\s+)?\d{2}/\d{2}/\d{4}(\s+)?to(\s+)?\d{2}/\d{2}/\d{4})', self.pdf_text)[0]
