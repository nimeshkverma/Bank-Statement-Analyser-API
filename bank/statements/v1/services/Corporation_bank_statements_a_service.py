...
HEADER = set(['Txn. Date', 'Particulars', 'Chq no.',
              'Txn.Type', 'Withdrawal', 'Deposit', 'Balance'])

MAX_START_DAY_OF_MONTH = 5
MIN_END_DAY_OF_MONTH = 25


class CorporationBankStatementsA(object):
    """Class to analyse the data obtained from Corporation Bank Type A"""
    ...
    def __get_pdf_dates(self):
        ...
        try:
            from_to_string_date_list = re.findall(
                r'(From Date(\s+)?:(\s+)?\d{2}/\d{2}/\d{4}(\s+)?To Date(\s+)?:(\s+)?\d{2}/\d{2}/\d{4})', self.pdf_text)[0]
    ...
    def __get_first_day_balance(self):
        ...
        try:
            for data_list in self.raw_table_data.get('body', []):
                if len(data_list) in [3] and "opening balance" in data_list[1].lower():
                    balance = self.__get_amount(data_list[-1])
                    break
        except:
            pass
