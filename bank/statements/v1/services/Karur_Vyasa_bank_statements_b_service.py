...
HEADER = set(['Transaction Date', 'Value Date', 'Branch',
              'Cheque No', 'Description', 'Debit', 'Credit', 'Balance'])
...
class KarurVyasaBankStatementsB(object):
    """Class to analyse the data obtained from Karur Vyasa Bank Type B"""
    ...
    def __get_date(self, date_input):
        ...
        for date_regex in [r'(\d{2}-[a-zA-Z]{3}-\d{4})']:
    ...
    def __get_statement(self, data_list):
        statement_dict = {}
        try:
            if self.__get_date(data_list[1]):
                statement_dict.update({
                    'transaction_date': self.__get_date(data_list[1]),
                    'description': ' '.join(data_list[1:-2]) if ' '.join(data_list[1:-2]) else 'Not Found',
                    'withdraw_deposit': self.__get_amount(data_list[-2]) if self.__get_amount(data_list[-2]) else 0,
                    'balance': self.__get_amount(data_list[-1]) if self.__get_amount(data_list[-1]) else 0,
                })
        except Exception as e:
    ...
    def __get_first_day_balance(self):
        ...
        try:
            balance_string_list = re.findall(
                r'INR\s*([0-9,]+\.\d{2})\s*[0-9,]+\.\d{2}\s*Specify\s*Period', self.pdf_text)
            for balance_string in balance_string_list:
                balance = self.__get_amount(balance_string)
                break
        except:
            pass
    ...
    def data_json(self, threshhold):
        data = {
            ...
            'bank_name': 'Karur Vyasa Bank Type B',
        }
        return data
