...
class AndraBankStatementsA(object):
    """Class to analyse the data obtained from Andra Bank Type A"""
    ...
    def __get_pdf_dates(self):
        ...
        try:
            from_to_string_date_list = re.findall(
                r'(Account Statement of(\s+)?\d*(\s+)?from(\s+)?\d{2}-\d{2}-\d{4}(\s+)?to(\s+)?\d{2}-\d{2}-\d{4})', self.pdf_text)[0]
    ...
    def __get_first_day_balance(self):
        ...
        try:
            for data_list in self.raw_table_data.get('body', []):
                if len(data_list) in [1, 2] and "balance" in data_list[0].lower():
                    balance = self.__get_amount(data_list[-1])
                    break
        except:
            pass
